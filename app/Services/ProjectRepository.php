...
class ProjectRepository
{
    ...
    public function create($data)
    {
        $project = $this->project->create([
            'name' => $data['name'],
            'slug' => Arr::get($data, 'slug', str_slug($data['name'])),
    ...
    public function applyTemplate($templateData, $projectPath, $overrideFiles = true)
    {
        ...
        foreach ($this->filesystem->allFiles("$this->templatesPath/$templateName") as $fileInfo) {
            ...
            if (str_contains($filePath, 'styles.css')) continue;

            // don't copy over template config file
            if (str_contains($filePath, 'config.json')) continue;
    ...
    {
        return $this->filesystem->get($this->getBuilderAssetPath($path));
    }

    private function buildAssetExists($path)
    {
        return $this->filesystem->exists($this->getBuilderAssetPath($path));
    }

    private function getBuilderAssetPath($path)
    {
        $path = strtolower($path);
        return public_path("builder/$path");
    }
}
